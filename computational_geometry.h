...
int cmp(db k1, db k2); // return -1 if k1<k2 else 1 if k1>k2 else 0
int inmid(db k1,db k2,db k3); // k3 在 [k1,k2] 内 
int sign(db k); // return -1 if k < 0 else 1 if k > 0 else 0
...
struct point{
    ...
    db abs(){return sqrt(x*x+y*y);}//长度
    db abs2(){return x*x+y*y;}//长度^2
    db dis(point k1){return ((*this)-k1).abs();}
    point unit(){db w=abs(); return (point){x/w,y/w};}
    void scan(){double k1,k2; scanf("%lf%lf",&k1,&k2); x=k1; y=k2;}
    void print(){printf("%.11lf %.11lf\n",x,y);}
    db getw(){return atan2(y,x);} //角度
    point getdel(){if (sign(x)==-1||(sign(x)==0&&sign(y)==-1)) return (*this)*(-1); else return (*this);}
    int getP() const{return sign(y)==1||(sign(y)==0&&sign(x)==-1);}
};

int checkLL(point k1,point k2,point k3,point k4);// 判断直线 k1,k2 和 k3,k4 有无交点
int checkSS(point k1,point k2,point k3,point k4);// 判断线段相交
int clockwise(point k1,point k2,point k3);// 213 逆时针 1 顺时针 -1 否则 0
int compareangle (point k1,point k2);//比较角度, 1 if k1<k2，范围(-180, 180]
db cross(point k1,point k2);//叉积
db disSP(point k1,point k2,point q);//点到线段最短距离
db disSS(point k1,point k2,point k3,point k4);//线段最短距离
db dot(point k1,point k2);//点积
point getLL(point k1,point k2,point k3,point k4);// 求直线 k1,k2 和 k3,k4 的交点 
int inmid(point k1,point k2,point k3);// k1.x<=k3.x<=k2.x && k1.y<=k3.y<=k2.y
int intersect(db l1,db r1,db l2,db r2);// 判断两线段相交
int onS(point k1,point k2,point q);// q在线段k1k2上
point proj(point k1,point k2,point q);// q 到直线 k1,k2 的投影 
db rad(point k1,point k2);//两个点形成的角度
point reflect(point k1,point k2,point q);// 以k1k2为对称轴，q的对称点
